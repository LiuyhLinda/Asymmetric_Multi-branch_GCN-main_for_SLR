...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/WLASL/train/train_bone_motion.yaml',
...
class Processor():
    ...
    def print_log(self, str, print_time=True):
        ...
        if self.arg.print_log:
            with open('{}/log'.format(self.arg.work_dir)+arg.flag+'.txt', 'a') as f:
                print(str, file=f)
    ...
    def train(self, epoch, save_model=False):
        ...
        torch.save(weights, self.arg.model_saved_name + arg.flag + '.pt')
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                for batch_idx, (data, label, index) in enumerate(process):
                    ...
                    score_frag.append(output.data.cpu().numpy())  # output.data [10, 2000]
                    loss_value.append(loss.data.cpu().numpy())

                    _, predict_label = torch.max(output.data, 1)
                    # print(predict_label.shape)  # --add torch64
                    step += 1
                ...
                arg.now_acc = accuracy
                if accuracy >= arg.best_acc:
                    arg.best_acc = accuracy
                    score_dict = dict(zip(self.data_loader[ln].dataset.sample_name, score))
                    print(arg.Experiment_name + "--Best accuracy:{}".format(arg.best_acc).center(60, '*'))
                    with open('./work_dir/' + arg.Experiment_name
                              + '/eval_results/best_acc' + arg.flag + '.pkl', 'wb') as f:
                ...
                if not arg.now_acc < arg.best_acc:
                    self.print_log(arg.Experiment_name + arg.flag+" "+"Best acc:{}".format(arg.best_acc))  # --add
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            print('best accuracy: ', arg.best_acc,
